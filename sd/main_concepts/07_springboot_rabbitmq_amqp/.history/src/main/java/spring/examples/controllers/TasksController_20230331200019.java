...
import spring.examples.services.HttpRequests;
...
public class TasksController {
	...
	public ResponseEntity<?> create (@RequestBody String task){
        ...
        try {
			...
			String docker_container = "docker run --rm -d --name="+t.getTaskName()+"-"+randomNum+" -p "+randomNum+":8080 "+ t.getFullContainerImage();
...
// so add 1 to make it inclusive
        ...
        return new ResponseEntity<String>("Task stored "+response.body().trim(), HttpStatus.CREATED);
    }
}
